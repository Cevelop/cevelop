...
class IndexNameToAstNameMatcher extends ASTVisitor {
    ...
    private boolean matchesIndexName(IASTName candidate) {
        ...
        return locationToFind.getNodeOffset() == candidateLocation.getNodeOffset() && locationToFind.getNodeLength() == candidateLocation
                .getNodeLength() && locationToFind.getFileName().equals(candidateLocation.getFileName()) && CharArrayUtils.equals(candidate
                        .getLookupKey(), charNameToFind);
    }
...
class BindingToAstNameMatcher extends ASTVisitor {
    ...
    public BindingToAstNameMatcher(IBinding binding, IIndex index) {
        super(true);
        bindingToFind = BindingHelper.adaptBinding(binding, index);
        this.index = index;
    ...
    private boolean isEquivalent(IASTName candidate) {
        return CharArrayUtils.equals(candidate.getLookupKey(), toFindName) && bindingToFind.equals(BindingHelper.adaptBinding(candidate
                .resolveBinding(), index));
    }
