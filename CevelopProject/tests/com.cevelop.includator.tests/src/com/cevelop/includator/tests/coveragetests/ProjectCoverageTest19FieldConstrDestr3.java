...
public class ProjectCoverageTest19FieldConstrDestr3 extends IncludatorTest {
    ...
    public void runTest() throws Throwable {
        ...
        assertSuggestion(suggestions.get(i++), "A.cpp", implicitlyInUseText, 37, 10); // Y::Y() { }
        assertSuggestion(suggestions.get(i++), "A.cpp", implicitlyInUseText, 83, 4); // X();
        assertSuggestion(suggestions.get(i++), "A.cpp", implicitlyInUseText, 98, 4); // Y y;
        assertSuggestion(suggestions.get(i++), "A.cpp", implicitlyInUseText, 107, 10); // X::X() { }
        assertSuggestion(suggestions.get(i++), "A.cpp", inUseText, 62, 7); // class X
        assertSuggestion(suggestions.get(i++), "A.cpp", inUseText, 132, 44); // int main() {
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 27, 5); // ~Y();
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 49, 11); // Y::~Y() { }
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 90, 5); // ~X();
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 119, 11); // X::~X() { }
    }
}
