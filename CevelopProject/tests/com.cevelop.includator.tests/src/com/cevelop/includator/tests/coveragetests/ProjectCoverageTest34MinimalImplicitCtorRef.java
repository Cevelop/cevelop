...
public class ProjectCoverageTest34MinimalImplicitCtorRef extends IncludatorTest {
    ...
    public void runTest() throws Throwable {
        ...
        assertSuggestion(suggestions.get(i++), "A.cpp", implicitlyInUseText, 69, 4); // Y y;
        assertSuggestion(suggestions.get(i++), "A.cpp", inUseText, 41, 7); // class X
        assertSuggestion(suggestions.get(i++), "A.cpp", inUseText, 90, 47); // int main() {
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 20, 4); // Y();
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 29, 10); // Y::Y() { }
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 62, 4); // X();
        assertSuggestion(suggestions.get(i++), "A.cpp", notInUseText, 78, 10); // X::X() { }
    }
}
