...
public class SuppressSuggestionTest6OneIgnoredAndOneSuggestedCoveredInclude extends IncludatorTest {

    @Test
    public void runTest() throws Throwable {
        ResourceWrapperStore store = new ResourceWrapperStore();
        store.setResource(getActiveProject().getCProject().getProject());
        store.setValue(IncludatorPropertyManager.SUPPRESSED_SUGGESTIONS_PROPERTY_NAME, "main.cpp: C.h");

        MultiStatus status = runAlgorithmsAsAction(new OrganizeIncludesAlgorithm(), AlgorithmScope.FILE_SCOPE);
        String expectedMsg = "Prevented adding of suggestion 'The include statement '#include \"C.h\"' is unneeded. Covered through: A.h' " +
        ...
        assertSuggestion(suggestions.get(0), "main.cpp", "The include statement '#include \"B.h\"' is unneeded. Covered through: A.h", 15, 14);
